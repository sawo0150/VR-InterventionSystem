...
public class ObstacleCollisionHandler : MonoBehaviour
{
    ...
    public ObstacleType obstacleType = ObstacleType.Deer;

    [Header("Detection Settings")]
    [Tooltip("Use trigger detection (deer) or collision detection (rolling stone)")]
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        Debug.Log($"[ObstacleCollisionHandler] *** TRIGGER DETECTED *** on {gameObject.name} with {other.gameObject.name}");
    ...
    private void HandleDetection(Transform detectedTransform)
    {
        ...
        if (isRobot)
        {
            ...
            Event1Controller controller = FindFirstObjectByType<Event1Controller>();
            if (controller != null)
            {
                controller.RespawnRobot(obstacleType);
            }
            else
            {
        ...
        else if (enableDebugLogs)
        {
            ...
            Debug.Log($"[ObstacleCollisionHandler] Non-robot detection on {gameObject.name} with {detectedTransform.gameObject.name} (tag: {detectedTransform.gameObject.tag})");
        }
    }
