...
public class Event2EndTrigger : MonoBehaviour
{
    ...
    void OnRobotReachedEnd(GameObject robot)
    {
        ...
        if (enableDebugLogs)
        {
            Debug.Log($"[Event2EndTrigger] Robot reached end zone at {transform.position}");
        }
    ...
    void ShowResetButton()
    {
        if (resetButton != null)
        {
            ...
            if (enableDebugLogs)
            {
                Debug.Log($"[Event2EndTrigger] Reset button shown");
            }
        }
    }
    ...
    public void OnResetButtonClicked()
    {
        if (enableDebugLogs)
        {
            Debug.Log($"[Event2EndTrigger] Reset button clicked");
        }
