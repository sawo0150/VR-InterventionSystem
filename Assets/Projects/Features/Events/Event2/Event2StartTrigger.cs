...
/// </summary>
public class Event2StartTrigger : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Debug.LogError($"[Event2StartTrigger] No collider found on {gameObject.name}!");
        }
        else if (!triggerCollider.isTrigger)
        {
            Debug.LogError($"[Event2StartTrigger] Collider on {gameObject.name} is NOT set as trigger! Check 'Is Trigger' checkbox.");
        }
        else if (enableDebugLogs)
        {
            Debug.Log($"[Event2StartTrigger] Trigger setup verified on {gameObject.name}");
        }
    }
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (enableDebugLogs)
        {
            Debug.Log($"[Event2StartTrigger] Trigger entered by: '{other.gameObject.name}' (Tag: '{other.tag}')");
        }
        ...
        else if (enableDebugLogs)
        {
            ...
            {
                Debug.LogWarning($"[Event2StartTrigger] Tag mismatch! Expected '{robotTag}', got '{other.tag}'");
            }
            if (eventStarted)
            {
                Debug.LogWarning($"[Event2StartTrigger] Event already started, ignoring trigger");
            }
        }
    }
    ...
    void StartEvent(GameObject robot)
    {
        ...
        if (enableDebugLogs)
        {
            Debug.Log($"[Event2StartTrigger] Robot arrived at event location");
        }
    ...
    public void ResetEvent()
    {
        ...
        if (enableDebugLogs)
        {
            Debug.Log($"[Event2StartTrigger] Event reset");
        }
    }
