...
/// </summary>
public class Event3EndTrigger : MonoBehaviour
{
    [Header("UI Settings")]
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Robot") && !eventCompleted)
        {
    ...
    void OnRobotReachedEnd(GameObject robot)
    {
        ...
        // ∑Œ∫ø ¡∂¿€ ≤Ù±‚
        var controller = robot.GetComponent<RobotNavMeshController>();
        if (controller != null)
            controller.enabled = false;

        if (enableDebugLogs)
            Debug.Log($"[Event3EndTrigger] Robot reached end zone at {transform.position}");
    ...
    void ShowResetButton()
    {
        if (resetButton != null)
        {
            ...
            if (enableDebugLogs)
            {
                Debug.Log($"[Event3EndTrigger] Reset button shown");
            }
        }
    }
    ...
    public void OnResetButtonClicked()
    {
        if (enableDebugLogs)
        {
            Debug.Log($"[Event3EndTrigger] Reset button clicked");
        }
