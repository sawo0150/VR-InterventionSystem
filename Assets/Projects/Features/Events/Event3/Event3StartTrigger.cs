...
/// <summary>
/// Triggers Event 3 start when robot enters the zone.
/// Notifies SimulationSceneManager when robot arrives.
/// </summary>
public class Event3StartTrigger : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Debug.LogError($"[Event3StartTrigger] No collider found on {gameObject.name}!");
        }
        else if (!triggerCollider.isTrigger)
        {
            Debug.LogError($"[Event3StartTrigger] Collider on {gameObject.name} is NOT set as trigger! Check 'Is Trigger' checkbox.");
        }
        else if (enableDebugLogs)
        {
            Debug.Log($"[Event3StartTrigger] Trigger setup verified on {gameObject.name}");
        }
    }
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (enableDebugLogs)
        {
            Debug.Log($"[Event3StartTrigger] Trigger entered by: '{other.gameObject.name}' (Tag: '{other.tag}')");
        }
        ...
        else if (enableDebugLogs)
        {
            ...
            {
                Debug.LogWarning($"[Event3StartTrigger] Tag mismatch! Expected '{robotTag}', got '{other.tag}'");
            }
            if (eventStarted)
            {
                Debug.LogWarning($"[Event3StartTrigger] Event already started, ignoring trigger");
            }
        }
    }
    ...
    void StartEvent(GameObject robot)
    {
        ...
        if (enableDebugLogs)
        {
            Debug.Log($"[Event3StartTrigger] Robot arrived at event location");
        }
    ...
    public void ResetEvent()
    {
        ...
        if (enableDebugLogs)
        {
            Debug.Log($"[Event3StartTrigger] Event reset");
        }
    }
